	...
	public function nerTagger()
	{
		...
		else
		{
			...
			$url = "https://www.googleapis.com/freebase/v1/search?query=".$query. 
					"&key=AIzaSyDbC5DOOtvhgBa-3EnLTLl7uAejrVekC3M&indent=true".
					"&limit=1&output=(description)";

			$freebase = json_decode($this->Nerprocess->curlGetInfo($url));
			$data = $freebase->result;
			$sentence = "";
			for($i = 0; $i < count($data); $i++)
			{
				//$sentence .= $data[$i]->name;
				$content = "/common/topic/description";
				$description = $data[$i]->output->description->$content;
