...
class Nerprocess extends CI_Model
{
	...
	public function pregMatch($type, $result)
	{
		if($type == 0)
		{
			...
			preg_match_all("/\&lt;MISC\&gt;(.*?)\&lt;\/MISC/", $result, $misc, PREG_PATTERN_ORDER);
		...
		else
		{
			...
			{
				preg_match_all("/\&lt;LOCATION\&gt;(.*?)\&lt;\/LOCATION/", $array[$i], $location, PREG_PATTERN_ORDER);
				preg_match_all("/\&lt;PERSON\&gt;(.*?)\&lt;\/PERSON/", $array[$i], $person, PREG_PATTERN_ORDER);
				preg_match_all("/\&lt;ORGANIZATION\&gt;(.*?)\&lt;\/ORGANIZATION/", $array[$i], $organization, PREG_PATTERN_ORDER);
				preg_match_all("/\&lt;MISC\&gt;(.*?)\&lt;\/MISC/", $array[$i], $misc, PREG_PATTERN_ORDER);
				...
				if($misc[1]) $entity[$i]["misc"] = $misc[1];
			}
